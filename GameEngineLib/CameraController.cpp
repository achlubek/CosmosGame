    ...
    : internalCamera(new Camera()), position(glm::dvec3(0.0)), activeViewStrategy(new ManualCameraStrategy())
{
    ...
    internalCamera->createProjectionPerspective(90.0f, resolution.x / resolution.y, 0.00001f, 600.0f);
}
...
{
    auto resolution = AbsGameContainer::getInstance()->getResolution();
    internalCamera->createProjectionPerspective(fov, resolution.x / resolution.y, 0.00001f, 600.0f);
}
