...
vec4 celestialThickAtmosphereGetColorRoughnessMap(RenderedCelestialBody body, float height, vec3 dir){
    ...
    vec3 newRgbColor = mix(baseColor, vec3(length(baseColor)), varianceSaturation) * varianceSaturation * (vec3(varianceR, varianceG, varianceB) * 0.5 + 0.5);//hsv2rgb(newHsvColor);
