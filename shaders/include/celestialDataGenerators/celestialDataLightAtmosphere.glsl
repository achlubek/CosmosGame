...
vec4 celestialLightAtmosphereGetColorRoughnessMap(RenderedCelestialBody body, float height, vec3 dir){
    ...
    vec3 colorrandomizer = (1.0 - body.sufraceMainColor) * (getwaves(dir * 1.0, 5.0, 0.0, sin(body.seed ))* 0.5 + getwaves(dir * 2.0 * noise4d(vec4(dir, sin(body.seed ))), 15.0, 0.0, sin(body.seed ))* 0.25 + noise4d(vec4(dir, sin(body.seed ))) * 0.125);
